...
export function CategoryListPage() {
  ...
  return (
    <div style={{ padding: 16, background: '#0f172a', minHeight: '100vh' }}>
      ...
      {categories?.map(cat => (
        <div key={cat.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', background: '#1e293b', borderRadius: 8, marginBottom: 8, border: `2px solid ${cat.color ?? '#c4b5fd'}` }}>
